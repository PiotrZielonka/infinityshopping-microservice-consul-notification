...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'createTime'> & {
...
export class NotificationService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/notifications', 'infinityshoppingnotificationapp');
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      createTime: notification.createTime?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      createTime: restNotification.createTime ? dayjs(restNotification.createTime) : undefined,
    };
  }
